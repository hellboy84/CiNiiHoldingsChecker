...
8. 「登録済み資料を全て削除」は**表示中のパターンの登録資料のみ**を削除します（他のパターンの登録は残ります）
...
- カンマ（`,` `，` `、`）で区切ると複数の巻次を OR 条件で指定できます
- 巻次を入力した場合、**巻次の情報を持たない館は対象外**になります（単巻本などで巻次が登録されていない書誌は、巻次を空白にして使ってください）
- 入力欄をクリックすると、**そのページに実在する巻次**が入力候補として表示されます
- 候補から選んだあと、もう一度入力欄をクリックして次の候補を選ぶと**OR 条件として追加**されます（`上` を選んでから `下巻` を選ぶと `上,下巻` になります）。選択済みの巻次は候補から外れます
